...
本项目源码托管在 GitHub，但以下文件**不包含在仓库中**，每次在全新 Replit 环境部署时必须由 agent 从零初始化：

| 不包含的内容 | 说明 |
|---|---|
| `.replit` | Replit 工作流配置，由 `configureWorkflow()` 生成 |
| `artifacts/api-portal/.replit-artifact/artifact.toml` | 前端 artifact 注册文件，由 `verifyAndReplaceArtifactToml()` 生成 |
...
在 `code_execution` 沙箱中依次执行（每条 await 后再执行下一条）：
...
`artifacts/api-portal/.replit-artifact/` 目录在 GitHub 中不存在，需先创建目录再注册：

```bash
mkdir -p artifacts/api-portal/.replit-artifact
```
...
在 `code_execution` 中配置后端工作流（console 类型）：
...
# 期望: providers.openai.configured=true, providers.anthropic.configured=true 等

# 前端可访问性（检查 HTML 返回）
curl http://localhost:24927/ | grep '<title>'
...
`.replit` 文件中的完整工作流配置（供参考，由工具自动生成）：
